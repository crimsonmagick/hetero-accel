...
def main():
    ...
    parser.add_argument('--results-file', '--resfile', 
                        default=os.path.join(project_dir, 'src', 'evaluation', 'data', 'sota.csv'),
    ...
    colors_per_method = ['darkgreen', 'lightsteelblue', 'grey']
    hatches = ['//', 'xx', '..']
    ...
    group_names = [f'W{workload_id}' for workload_id in workloads]
    ...
    for ax, metric, letter in zip(axes, evaluated_metrics, letters):
        ...
        heights = [
            [
                df.loc[(df['Method'] == method) & (df['WorkloadID'] == workload_id)][metric].iloc[0]
        ...
        cfg_each_subplot(axes=ax,
                         ...
                         in_group_labels=evaluated_methods,
                         colors=colors_per_method,
                         hatches=hatches,)

        # subfigure title
        title_text = rf'\textbf{{({letter}) {metric.capitalize()}}}' if letter != '' else \
                     rf'\textbf{{{metric.capitalize()}}}'
    ...
    savefile = os.path.join(project_dir, 'results', 'figures', 'comp_sota_per_metric.pdf')
