...
def create_model(arch, dataset, batch_size=256, pretrained=True, parallel=True, device_ids=None, verbose=True):
    ...
    try:
        create_model_f, model_type, input_shape = {
            ...
            'voc_seg': (create_image_segmentation_model, DNNType.SemanticSegmantation, None),
            'voc_det': (create_object_detection_model, DNNType.ObjectDetection, (batch_size, 3, 320, 320)),
            'coco': (create_object_detection_model, DNNType.ObjectDetection, None),
            'sst2': (create_language_processing_model, DNNType.TextClassification, None),
            'multi30k': (create_language_processing_model, DNNType.MachineTranslation, None),
